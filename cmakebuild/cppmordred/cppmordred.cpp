...
namespace RDKit {
  ...
  std::vector<int> calcAromatic(const ROMol& mol) {
    ...
    // TODO: need debug
    std::vector<double> calcCarbonTypes(const RDKit::ROMol &mol, int version =1) {
        int slots = 10;
        if (version>1) { 
            slots = 11;
        };
...
std::vector<double> calcDetourMatrixDescsL(const RDKit::ROMol& mol) {
 ...
 //// AdjacencyMatrix 
    ...
    std::vector<double> calcAdjMatrixDescs(const RDKit::ROMol& mol, int version =1) {
        Eigen::MatrixXd AdjMat = calculateAdjacencyMatrix( mol);
    ...
    */
    ...
    std::vector<double> calcAdjMatrixDescsL(const RDKit::ROMol& mol, int version=1) {
...
*/
    ...
    std::vector<double> calcDistMatrixDescs(const RDKit::ROMol& mol, int version=1) {
...
std::vector<double> calcDistMatrixDescsL(const RDKit::ROMol& mol, int version=1) {
...
BOOST_PYTHON_MODULE(cppmordred) {
  ...
  boost::python::def("CalcTopoPSA", RDKit::calcTopoPSA); 
  boost::python::def("CalcSLogP", RDKit::calcSLogP); 
  boost::python::def("CalcHydrogenBond", RDKit::calcHydrogenBond); 
  boost::python::def("CalcLogS", RDKit::calcLogS); 
  boost::python::def("CalcLipinski", RDKit::calcLipinskiGhose); 
  boost::python::def("CalcMcGowanVolume", RDKit::calcMcGowanVolume); 
  boost::python::def("CalcPolarizability", RDKit::calcPolarizability); 
  boost::python::def("CalcRotatableBond", RDKit::calcRotatableBond); 
  boost::python::def("CalcFragmentComplexity", RDKit::calcFragmentComplexity); 
  boost::python::def("CalcConstitutional", RDKit::calcConstitutional); 
  boost::python::def("CalcTopologicalIndex", RDKit::calcTopologicalIndex); 

  boost::python::def("CalcDetourMatrixEigen", RDKit::calcDetourMatrixDescs); 
  boost::python::def("CalcDetourMatrix", RDKit::calcDetourMatrixDescsL); 

  boost::python::def("CalcDistanceMatrixEigen", RDKit::calcDistMatrixDescs); 
  boost::python::def("CalcDistanceMatrix", RDKit::calcDistMatrixDescsL); 


  boost::python::def("CalcAdjacencyMatrixEigen", RDKit::calcAdjMatrixDescs); 
  boost::python::def("CalcAdjacencyMatrix", RDKit::calcAdjMatrixDescsL); 

  boost::python::def("CalcCarbonTypes", RDKit::calcCarbonTypes);
  boost::python::def("CalcEccentricConnectivityIndex", RDKit::calcEccentricConnectivityIndex); 

  boost::python::def("CalcBaryszMatrix", RDKit::calcBaryszMatrixDescsL);
  boost::python::def("CalcBaryszMatrixEigen", RDKit::calcBaryszMatrixDescs);

  boost::python::def("CalcZagrebIndex", RDKit::calcZagrebIndex);
  boost::python::def("CalcMoeType", RDKit::calcMoeType); 
  boost::python::def("CalcMolecularDistanceEdge", RDKit::calcMolecularDistanceEdgeDescs); 
  boost::python::def("CalcEState", RDKit::calcEStateDescs); 
  ...
  boost::python::def("CalcPathCount", RDKit::calcPathCount); 
  boost::python::def("CalcKappaShapeIndex", RDKit::calcKappaShapeIndex); // closer "missing" k3 path count not correct on few cases


  boost::python::def("CalcRingCount", RDKit::calcRingCount); 
  boost::python::def("CalcMolecularId", RDKit::calcMolecularId); 
  //boost::python::def("CalcBCUTEigen", RDKit::calcBCUTs); 
  boost::python::def("CalcBCUT", RDKit::calcBCUTs); // 10x faster the
...
// for debug proposed only
  ...
  boost::python::def("CalcChiPath", RDKit::calcChiPath); // test function not used in mordred descriptors can be removed 
  boost::python::def("CalcChichain", RDKit::calcChichain); 
  boost::python::def("CalcChicluster", RDKit::calcChicluster); 
  boost::python::def("CalcChipathcluster", RDKit::calcChipathcluster);


  boost::python::def("CalcAcidicGroupCount", RDKit::calcAcidicGroupCount);
  boost::python::def("CalcBasicGroupCount", RDKit::calcBasicGroupCount);
  boost::python::def("CalcCountAromaticAtoms", RDKit::countAromaticAtoms);
  boost::python::def("CalcCountAromaticBonds", RDKit::countAromaticBonds);


  // Additional EState type features from OEState and Others 
  // extending EState ... Done let analysis the charges effect one day... 
  // add BEStates ... Done
  // add HEStates ... Done
  // add alphaKappa ... Done
  // add Abrahams ... Done
  // add Flexibility, RNCG, RRPG, Schultz, MR and Pol ... Done
  // add ODT ... todo

  boost::python::def("CalcBEState", RDKit::calcBEStateDescs); 
  boost::python::def("CalcHEState", RDKit::calcHEStateDescs); 
  boost::python::def("CalcAlphaKappaShapeIndex", RDKit::calcAlphaKappaShapeIndex); // include alpha version ... we could join the two codes 
  boost::python::def("CalcAbrahams", RDKit::calcAbrahams); // Platts, Butina, Abraham, Hersey  paper J Chem Inf Comput Sci. 1999 30/8/01;39(5):835-45
  boost::python::def("CalcPol", RDKit::calcPol); 
